...
interface QRCodeChatsViewProps {
  qrCodeId: string
}

export default function QRCodeChatsView({ qrCodeId }: QRCodeChatsViewProps) {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const [chatsResponse, qrResponse] = await Promise.all([
        fetch(`/api/admin/qr/${qrCodeId}/chats`),
        fetch(`/api/admin/qr/${qrCodeId}`).catch(() => null),
      ])
            ...
            <div>
              ...
              {qrCode && (
                <div className="mt-2">
                  ...
                  <p className="text-sm text-gray-600">
                    <strong>Owner:</strong> {qrCode.user.name || qrCode.user.email}
                  </p>
                </div>
              )}
            </div>
